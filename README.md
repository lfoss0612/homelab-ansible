...
| `playbooks/audit-claude-access.yml`, `deploy-ssh-access-logging.yml`, `setup-claude-audit-zabbix-items.yml`, `setup-claude-audit-timer.yml` | Written for the Claude Code access-control audit/monitoring layers (`docs/CLAUDE-INTEGRATION-SUMMARY.md`) — SSH access logging (`claude`/`openclaw` only, scoped via `LOGGED_USERS` in `files/ssh-access-wrapper.sh`), Zabbix trapper items, and a weekly audit timer. **Not yet run against any live host** — see `homelab-vault/TODO.md` → "Ansible — Claude access control deployment". |

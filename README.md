...
| `playbooks/deploy-claude-ssh-restrictions.yml` | Templates `/etc/ssh/sshd_config.d/10-claude-restrictions.conf` on `cockpit.home.lan`: `LogLevel VERBOSE` (server-side audit trail for all users) + a `Match User claude` block (publickey-only, no forwarding). Codifies a config first applied by hand during incident recovery — see `homelab-vault` `Incidents/2026-09-23-ssh-wrapper-broke-desktop-access.md`. Replaces the abandoned global-`ssh`-wrapper and client-side `LocalCommand` approaches; **not yet extended to `zabbix.home.lan`**. |
| `playbooks/audit-claude-access.yml`, `setup-claude-audit-zabbix-items.yml`, `setup-claude-audit-timer.yml` | Written for the Claude Code access-control audit/monitoring layers (`docs/CLAUDE-INTEGRATION-SUMMARY.md`) — Zabbix trapper items and a weekly audit timer. **Not yet run against any live host** — see `homelab-vault/TODO.md` → "Ansible — Claude access control deployment". |
| `playbooks/manage-zabbix-server-conf.yml` | Targeted `lineinfile` edits to specific `/etc/zabbix/zabbix_server.conf` settings (via `-e zabbix_server_conf_settings=...`), reloading the config cache on change. Runs as `ansible` (already has unconditional root everywhere) — replaces the root sudoers grant `claude` used to hold on `zabbix.home.lan` for the same two operations. No-op by default (empty settings list). |
...
| `playbooks/smart-monitor.yml` | Installs `/usr/local/bin/smart-monitor`, its service and 30-minute timer on `[smart_monitored]` (`pbs`, `pve`, `pve-ai`, `pve-router`). Asserts `smartctl`/`zabbix_sender` are present rather than skipping a host that lacks them — the previous version gated on `/etc/zabbix/zabbix_agentd.conf`, which no longer exists anywhere since the agent2 migration, so it silently skipped all four hosts while reporting success. |
| `playbooks/smart-monitor-zabbix-setup.yml` | Server-side counterpart: per-host SMART discovery rule, item/trigger prototypes, aggregate items, value maps, per-class temperature macros and the aggregate triggers — including `nodata(smart.status,2h)`, without which a monitor that stops running is indistinguishable from healthy disks. Idempotent; derives its targets from `[smart_monitored]`. **Run both halves** — either alone is silently inert. |
| `playbooks/zabbix-smart-dashboard.yml` | Builds the "SMART Drive Health" dashboard: active SMART problems, per-host roll-up, and honeycombs giving one cell per drive for health, failed attributes and temperature. Widgets match items **by name pattern**, so renaming a prototype in the setup playbook silently empties a panel — change them together. |
| `playbooks/oom-kill-monitor.yml` | Installs `/usr/local/bin/oom-kill-monitor`, its service and 5-minute timer on `[oom_monitored]` (`pbs`, `pve`, `pve-ai`, `pve-router` — same members as `[smart_monitored]` today, but a separate group since VMs are the likely next addition). Tails the kernel journal incrementally via a stored cursor and reports any `Out of memory: Killed process` line, pairing it with the preceding `task_memcg=` line so a killed `kvm` process resolves to a VMID. Built 2026-08-27 after the kernel killed VM 105 (`k8s-worker`) on `pve.home.lan` with zero alerting anywhere. |
| `playbooks/oom-kill-monitor-zabbix-setup.yml` | Server-side counterpart: a trapper count item, a text item for the most recent kill, and per-host triggers — including `nodata(system.oom.count,1h)`, the same "a dead monitor looks like no problem" gap that let the 2026-08-24 kill go unnoticed. Idempotent; derives its targets from `[oom_monitored]`. **Run both halves.** |
| `playbooks/claude-remote-control.yml` | Installs the `claude-remote-control` systemd unit on `[claude_remote_control]` (`desktop` alone) — `~lfoss/.local/bin/claude remote-control`, `Type=simple`, `Restart=always`/`RestartSec=10`, `WorkingDirectory=/home/lfoss/projects`. The only service in this repo that runs as an interactive user off a binary in that user's home instead of as root off `/usr/local/bin`, so the play asserts the binary and the working directory exist rather than installing them: Claude Code is installed per-user by its own installer, and writing the unit without either one leaves systemd retrying a `203/EXEC` or `200/CHDIR` failure every 10 seconds forever. Explicit-invocation-only — deliberately not in the weekly convergence timer, so `desktop` being powered off is a visible failure here rather than a red timer. Adopted 2026-08-28 from a unit created by hand on the host.
| `playbooks/pbs-export-k8s-datastore.yml` | Creates and exports `backup10tb/k8s_backups` on the ZFS mirror, alongside `host_backups`/`photo_backups`/`vm_backups`/`longhorn_backups`. Locked to `0700` — it will hold cluster CA private keys and Secret-encryption keys, the same sensitivity class as OPNsense's internal CA key. Run before `k8s-etcd-backup.yml`. |
| `playbooks/k8s-etcd-backup.yml` | Installs an hourly timer on `[k8s_master]` that copies RKE2's etcd snapshots and its `server/` directory (minus the live `db/`) to that export — the off-node half of etcd disaster recovery that RKE2's own snapshot job doesn't provide on a single-control-plane cluster. Deliberately backs up the whole `server/` tree rather than a hand-picked file list: `cred/encryption-*.json` (without which a restored snapshot's Secrets are permanently unreadable) is easy to miss if you only think to grab `tls/`. |
